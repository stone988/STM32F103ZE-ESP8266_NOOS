...
bool ESP8266_Send_AT_Cmd(char *cmd,char *ack1,char *ack2,u32 time)
{
    ...
    USART2_DataFram.InfAll = 0; //重新接收新的数据包
    ESP8266_USART("%s\r\n", cmd);
...
{
    if (!ESP8266_Send_AT_Cmd ( "AT+CIPMODE=1", "OK", 0, 500 ))		//开启透传模式
        return false;

    return 
        ESP8266_Send_AT_Cmd( "AT+CIPSEND", "OK", ">", 500 );		//开始发送数据
...
bool ESP8266_SendString(FunctionalState enumEnUnvarnishTx, char * pStr, u32 ulStrLength, ENUM_ID_NO_TypeDef ucId )
{
    ...
    if ( enumEnUnvarnishTx )    //透传模式，直接发送
    {
...
static char *itoa( int value, char *string, int radix )
{
    ...
    /* This implementation only works for decimal numbers.(十进制) */
    if (radix != 10)
    {
        *ptr = 0;
        return string;
    ...
    /* if this is a negative value(负数) insert the minus sign. */
    if (value < 0)
    {
        *ptr++ = '-';
